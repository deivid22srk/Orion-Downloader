...
class NativeDownloadEngine {
    ...
    init {
        try {
            ...
            Log.d("NativeDownloadEngine", "Engine created: $engineId")
        } catch (e: Exception) {
            Log.e("NativeDownloadEngine", "Failed to load native library", e)
            engineId = 0
        }
    }
    
    fun isNativeAvailable(): Boolean = engineId != 0
    
    suspend fun getContentLength(url: String): Long = withContext(Dispatchers.IO) {
        if (engineId == 0) return@withContext -1L
        try {
    ...
    suspend fun supportsRangeRequests(url: String): Boolean = withContext(Dispatchers.IO) {
        if (engineId == 0) return@withContext false
        try {
